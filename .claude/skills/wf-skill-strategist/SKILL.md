...
See `references/brainstorm-patterns.md` for triggers (vague verbs, missing coverage, single-noun product mentions, common adjacencies, explicit asks) and the `question` tool shape. The output of every brainstorm is **proposed CAP-NNN candidates in user-voice**, never components, scope, or technology.

### Phase 4 — Readback

**This phase is non-negotiable, even under autonomy signals.** If the session is configured to "work without clarifying questions" or similar (harness reminders, user-issued autonomy modes), the right adaptation is to batch `question` tool calls more aggressively — 3–4 questions per call, fewer rounds — not to skip readback. Batched disambiguation IS the efficient form of interaction; it's not the "stopping to check" the autonomy signal is meant to suppress. Tactical clarifications inside intake (Phase 2) may be inferred away when the harness signals autonomy; bucket validation in Phase 4 cannot.

Now make the bucket call visible. For each item collected during intake + brainstorm, surface it via the `question` tool so the user can affirm or reframe.

Bucket → `header` mapping (≤12 chars) — maps to the `header` field on each question:
...
Group readback into batches (3–6 items per question session) so the user can stay oriented. Do **not** dump 30 items into one `question` call.

The `question` tool accepts a `questions` array. Each entry has `question`, `header`, and `options` (with `label`/`description`). Use the `header` values from the table above. The tool auto-adds a "Type your own answer" option — omit catch-all options.
