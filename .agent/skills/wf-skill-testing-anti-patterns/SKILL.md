...
## Quick reference and meta-rule

The 13 anti-patterns above are restated as one-line checks in `references/quick-reference.md`, alongside the three properties every good test must have. Load that reference when scanning an existing test file for issues, or as a fast recap during a review pass.
